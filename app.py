...
DEFAULT_SOUNDS = {
    "📚 เรียน": "sounds/learn.wav",
    "💼 งาน": "sounds/work.wav",
    "🎮 เล่น": "sounds/relax.wav",
    "🏃 สุขภาพ": "sounds/healht.wav"
}
...
def play_sound(path):
    ...
    real_path = resource_path(path) if not os.path.isabs(path) else path
...
def choose_sound():
    ...
    if path:
        ...
        sound_var.filename = os.path.basename(path)

def add_reminder():
    if not task_entry.get():
        return

    # 1) ถ้าผู้ใช้เลือกเสียงเอง → ใช้อันนั้น
    if hasattr(sound_var, "filename"):
     sound_path = f"sounds/{sound_var.filename}"
    else:
     sound_path = DEFAULT_SOUNDS.get(cat.get(), "")
    ...
    sound_var.set("เลือกเสียง (.wav)")
    if hasattr(sound_var, "filename"):
     del sound_var.filename
